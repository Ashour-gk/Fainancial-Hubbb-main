        ...
        {attachRowId !== null && (
          <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.5)',backdropFilter:'blur(4px)',zIndex:9999,display:'flex',alignItems:'center',justifyContent:'center',animation:'delFadeIn .2s ease'}} onClick={cancelAttach}>
            <div style={{background:'#fff',borderRadius:20,padding:'32px 28px 24px',width:'90%',maxWidth:360,textAlign:'center',boxShadow:'0 20px 60px rgba(0,0,0,.2)'}} onClick={e=>e.stopPropagation()}>
              ...
              <div style={{fontSize:'.88rem',color:'#64748b',marginBottom:16}}>أدخل اسم الملف المرفق</div>
              <input value={attachName} onChange={e=>setAttachName(e.target.value)} onKeyDown={e=>{if(e.key==='Enter')confirmAttach()}} autoFocus placeholder="مثال: فاتورة.pdf" style={{width:'100%',padding:'10px 14px',border:'1.5px solid #e2e8f0',borderRadius:12,fontSize:'.9rem',fontFamily:'inherit',outline:'none',direction:'rtl',marginBottom:20,boxSizing:'border-box',transition:'border-color .15s'}} />
